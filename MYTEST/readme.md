...
I thought it would be interesting to see if the v5.0.0-branch initializes
the hardware module very differently than the master.

I have not shared the changes made on the master "branch" but only the MYTEST in v5.0.0-branch.

I am sharing the 2 log files here anyway for a comparison between V5 and V4.

The log file shows how the hardware device is initialized in order to receive the first packet.
